...
> **30회 한정 강화(운/전략) + 직접 컨트롤 전투(피지컬)** 를 결합한 웹 기반 1vs1 2D 횡스크롤 액션 게임.
> 다크 판타지 × 네온 레트로 무드의 직관적·타격감 있는 UI로 구현했습니다.
...
- 총 **30회**의 강화 시도를 **검(공격력) · 방어구(체력/방어) · 능력치(기동성/재생)** 에 자유 분배.
- 각 부위 **0강 ~ 10강**, 강화 단계가 오를수록 성공 확률이 급감합니다.
- **5강부터 하락(다운) 확률** 적용 — 실패 시 단계가 떨어질 수 있습니다.
- **5 · 7 · 9 · 10강** 달성 시 특수 능력 아이콘이 점등·중첩됩니다.
...
| 3 | 강화 단계 | 남은 횟수 카운터, 검→방어구→능력치 3분할, 확률/특수능력 아이콘 |
| 4 | 인게임 전투 | 중앙 타이머, 양측 체력바, WASD/마우스/1·2·3 조작 HUD |
...
조작하고 **타격/사망 이벤트**를 상대에게 통보합니다. 시그널링 실패 시 자동으로 *연습 모드*
안내로 폴백됩니다.
